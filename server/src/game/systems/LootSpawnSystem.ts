...
function getLootTableForTier(tier: string): LootTableEntry[] {
  switch (tier) {
    case 'barrel': return BARREL_LOOT_TABLE;
    case 'basic': return BASIC_CRATE_LOOT_TABLE;
    case 'military': return MILITARY_CRATE_LOOT_TABLE;
    case 'elite': return ELITE_CRATE_LOOT_TABLE;
    default: return BARREL_LOOT_TABLE;
  }
}

function getHealthForTier(tier: string): number {
  switch (tier) {
    case 'barrel': return 10;
    case 'basic': return 50;
    case 'military': return 100;
  ...
  for (const spawn of spawnPoints) {
    ...
    if (spawn.entityId !== null) {
      if (world.ecs.entityExists(spawn.entityId)) {
        ...
        const health = world.ecs.getComponent<HealthComponent>(spawn.entityId, ComponentType.Health);
    ...
    if (spawn.entityId === null) {
      ...
      const items = rollLootTable(lootTable);
      if (items.length === 0) continue; // No items rolled, try next tick
      ...
      world.ecs.addComponent<PositionComponent>(entityId, ComponentType.Position, {
        x: spawn.position.x, y: spawn.position.y, z: spawn.position.z, rotation: 0,
      });
      ...
      world.ecs.addComponent(entityId, ComponentType.Collider, {
        width: 0.8, height: 0.8, depth: 0.8, isStatic: true,
      });
