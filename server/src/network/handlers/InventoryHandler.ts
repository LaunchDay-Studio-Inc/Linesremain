...
function isValidMovePayload(payload: unknown): payload is InventoryMovePayload {
  ...
  return (
    typeof p.fromSlot === 'number' && Number.isInteger(p.fromSlot) &&
    typeof p.toSlot === 'number' && Number.isInteger(p.toSlot)
  );
}
...
function isValidDropPayload(payload: unknown): payload is InventoryDropPayload {
  ...
  return (
    ...
    typeof p.quantity === 'number' && Number.isInteger(p.quantity) && p.quantity > 0
  );
}
...
export function registerInventoryHandlers(
  ...
  // ─── Inventory Move ───
  socket.on(ClientMessage.InventoryMove, (payload: unknown, callback?: (res: InventoryResponse) => void) => {
    ...
    if (payload.fromContainer !== undefined) {
      ...
      const containerPos = world.ecs.getComponent<PositionComponent>(containerId as EntityId, ComponentType.Position);
      ...
      sourceInv = world.ecs.getComponent<InventoryComponent>(containerId as EntityId, ComponentType.Inventory);
    ...
    if (payload.toContainer !== undefined) {
      ...
      destInv = world.ecs.getComponent<InventoryComponent>(containerId as EntityId, ComponentType.Inventory);
    } else {
    ...
    if (!isValidSlot(payload.fromSlot, sourceInv.maxSlots) ||
  ...
  // ─── Inventory Drop ───
  socket.on(ClientMessage.InventoryDrop, (payload: unknown, callback?: (res: InventoryResponse) => void) => {
    const playerId = getPlayerId(socket);
    if (!playerId) {
      callback?.({ success: false, error: 'Not authenticated' });
      return;
  ...
  // ─── Inventory Split ───
  socket.on(ClientMessage.InventorySplit, (payload: unknown, callback?: (res: InventoryResponse) => void) => {
    const playerId = getPlayerId(socket);
    if (!playerId) {
      callback?.({ success: false, error: 'Not authenticated' });
      return;
  ...
  socket.on(ClientMessage.Interact, (payload: unknown, callback?: (res: InventoryResponse) => void) => {
    const playerId = getPlayerId(socket);
    if (!playerId) {
      callback?.({ success: false, error: 'Not authenticated' });
      return;
    ...
    const targetInv = world.ecs.getComponent<InventoryComponent>(targetId, ComponentType.Inventory);
    if (!targetInv) {
